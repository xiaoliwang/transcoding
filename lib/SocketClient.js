...
function connect(IP, port, callback) {
  ...
  socket.on('connect_success', data => {
    logger.info(`[${socket.id}] [${IP}] connect server successed`);
    callback(data);
  });
  socket.on('disconnect', (reason) => {
      ...
      // socket.open();
      // 记录到日志
      logger.warn(`[${socket.id}] [${IP}] disconnect from transcoding server, reason: ${reason}`);
      // 发送到 QQ 消息通知
      sendQQMessage(`转码客户端断开 socket 连接, 原因: ${reason}`)
  });
  // 将音频压缩信息报告服务器
